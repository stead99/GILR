...
process_tn_pair() {
    ...
    # 使用 GATK MergeVcfs 将过滤后的SNV和INDEL合并
    # MergeVcfs需要参考基因组来确保顺序正确性，并生成索引 (.tbi)
    echo "Merging filtered SNVs and INDELs into a single VCF..."
    singularity exec $IMG_GATK gatk MergeVcfs \
        -I "${strelka_dir}/${pair_id}/results/variants/${pair_id}.snvs.PASS.vcf" \
        -I "${strelka_dir}/${pair_id}/results/variants/${pair_id}.indels.PASS.vcf" \
        -O "${strelka_dir}/${pair_id}/results/variants/${pair_id}.somatic.PASS.vcf" \
